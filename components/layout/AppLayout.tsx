...
export function AppLayout({ children }: AppLayoutProps) {
  ...
  // Handlers for sidebar, notifications and profile menu
  const handleSidebarToggle = () => setSidebarOpen((prev) => !prev);

  // Effect to close dropdowns when clicking outside (notifications or profile)
  React.useEffect(() => {
  ...
  // Effect to close dropdowns on route change
  React.useEffect(() => {
  ...
  const handleProfileAction = (href?: string, action?: string) => {
    if (action === "logout") {
      // TODO: implement logout logic as needed
                        ...
                        {PROFILE_MENU.map((item) => {
                          ...
                          return (
                            <button
                              key={item.label}
                              onClick={() =>
                                handleProfileAction(item.href, item.action)
                              }
